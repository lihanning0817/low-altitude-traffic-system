-- ============================================================================
-- 数据库迁移脚本: 修复 emergency_events 表以匹配后端代码要求
-- 创建时间: 2025-10-05
...
CREATE TABLE emergency_events (
    ...
    drone_id BIGINT NULL COMMENT '关联无人机ID（可选）',
    ...
    responded_by BIGINT NULL COMMENT '响应人员ID（可选）',
...
-- 4. 如果需要从备份恢复数据，可以执行以下语句（需要手动调整字段映射）
-- INSERT INTO emergency_events (event_code, task_id, drone_id, type, severity, status, title, description, location, created_at, updated_at)
-- SELECT
--     event_id as event_code,
--     task_id,
--     drone_id,
--     CASE event_type
--         WHEN 'mechanical_failure' THEN 'equipment_failure'
--         WHEN 'battery_critical' THEN 'battery_low'
--         WHEN 'lost_control' THEN 'signal_loss'
--         ELSE event_type
--     END as type,
--     severity,
--     CASE status
--         WHEN 'reported' THEN 'active'
--         WHEN 'investigating' THEN 'responding'
...
-- 5. 迁移完成后可以删除备份表
-- DROP TABLE IF EXISTS emergency_events_backup;
